...
```{r}
model2_hgwr_err <- c("Intercept", "g1", "g2", "z1", "x1") %>%
  sapply(function(coef, beta_true, beta_hat) {
    coef_true <- beta_true[[coef]]
    coef_hat <- beta_hat[[coef]]
    mae <- mean(abs(coef_true - coef_hat))
    rmse <- sqrt(mean((coef_true - coef_hat)^2))
...
model2_gwr_err <- c("Intercept", "g1", "g2", "z1", "x1") %>%
  sapply(function(coef, beta_true, beta_hat) {
    coef_true <- beta_true[[coef]]
    coef_hat <- beta_hat[[coef]]
    mae <- mean(abs(coef_true - coef_hat))
...
model2_hlm_err <- c("Intercept", "g1", "g2", "z1", "x1") %>%
  sapply(function(coef, beta_true, beta_hat) {
    coef_true <- beta_true[[coef]]
    coef_hat <- beta_hat[[coef]]
    mae <- mean(abs(coef_true - coef_hat))
...
```{r}
list(HGWR = model2_hgwr_err, GWR = model2_gwr_err, HLM = model2_gwr_err) %>% {
  lapply(names(.), function(algorithm, err_list, coef_list) {
    lapply(coef_list, function(coef, err) {
      data.frame(Algorithm = algorithm, Coefficient = coef,
                 Label = rownames(err), Value = unlist(err[[coef]]))
    }, err_list[[algorithm]]) %>% Reduce(rbind, .)
  }, ., c("Intercept", "g1", "g2", "z1", "x1")) %>% Reduce(rbind, .)
} %>%
...
It is ablt to give more precise estimations for local fixed effects,
global fixed effects and also random effects.
...
```{r, fig.height=7, fig.width=14, dpi=300}
